...
estimateShipEmissions2009 <- function(dAISInt, shipParameters, sampleGranularity, unit="g") {
    ...
    # Calculate PM EF for ME
    enginePowerME <- installedPowerME / n_installed_me
    nOperationalEngines <- calcNumOperativeEngines(transPME, enginePowerME)
    EL <- calcEngineLoad(transPME, enginePowerME, nOperationalEngines)
    relSFOC <- calcRelativeSFOC(EL)

    EFSO4 <- calcEmissionFactorSO4()
    EFH2O <- calcEmissionFactorH2O()
    OCEL <- calcOCELFactor(EL)
    PMFactME <- calcEmissionFactorPM(relSFOC, EFSO4, EFH2O, OCEL)
    ...
    PMME <- (PMFactME * transPME / 3600)/unit*sampleGranularity
